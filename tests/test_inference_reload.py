...
def test_inference_server_reload():
    ...
    assert abs(fingerprint1 - fingerprint2) >= 1e-6, (
        "Model weights are identical! Test setup failed."
    )
    ...
    assert abs(current_fingerprint - fingerprint1) <= 1e-6, (
        "InferenceServer doesn't match model1!"
    )
    ...
    print(
        f"[Test] InferenceServer after reload fingerprint: {reloaded_fingerprint:.6f}"
    )

    assert abs(reloaded_fingerprint - fingerprint2) <= 1e-6, (
        f"Reload failed! Expected: {fingerprint2:.6f}, Got: {reloaded_fingerprint:.6f}"
    )
