...
def test_replay_buffer():
    ...
    try:
        ...
        states = [np.zeros((9, 10, 10), dtype=np.float32) for _ in range(5)]
        policies = [np.zeros((10004,), dtype=np.float32) for _ in range(5)]
        values = [0, 1, -1, 1, 0]
        ...
        assert s.shape == (5, 9, 10, 10)
        assert p.shape == (5, 10004)
        assert v.shape == (5,)
